"""Budgeted sequential Optuna search around the Cora boundary optimum."""
import argparse
import optuna
import json
...
def main():
    ...
    p.add_argument('--output', default='/content/drive/MyDrive/GRIP_cora_teacher_fisher_optuna')
    ...
    with logfile.open('w', encoding='utf-8') as log:
        ...
        helpers = {name: sha(Path(__file__).with_name(name)) for name in (
            'teacher_fisher_optuna.py', 'teacher_fisher_metric.py', 'diagonal_metric.py',
        ...
        study_config = dict(base=base, selection=selection, confirmation=confirmation,
            ...
            space=dict(gamma=[1e-5,3e-5,1e-4,3e-4,.001], T=[.5,1.,2.],
                       mu=[3.,100.], alpha=[.002,.1]))
        ...
        study.enqueue_trial(dict(gamma=.001, T=1., mu=10., alpha=.05))
        ...
        for _ in range(total):
            ...
            mu = trial.suggest_float('mu', 3., 100., log=True)
            alpha = trial.suggest_float('alpha', .002, .1, log=True)
