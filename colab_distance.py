...
PRESET = os.environ.get('GRIP_SWEEP_PRESET', 'pilot')  # pilot / full / rho / weighted / robust
if PRESET not in ('pilot', 'full', 'rho', 'weighted', 'robust'):
    raise ValueError('GRIP_SWEEP_PRESET must be pilot, full, rho, weighted, or robust')
...
if PRESET == 'robust':
    OUTPUT = '/content/drive/MyDrive/GRIP_cora_robust_labels'
CASES = 'cora:0.052' if PRESET in ('weighted', 'robust') else 'cora:0.052,citeseer:0.036'
...
if PRESET == 'robust':
    command[command.index('--preset') + 1] = 'pilot'
    command[command.index('--methods') + 1] = 'robust'
    command[command.index('--baseline-kl') + 1] = '0.5'
    command[command.index('--dropouts') + 1] = '0.9'
    command += ['--gammas', '0.01', '--temperatures', '2',
                '--robust-caps', '0,0.01,0.03,0.1', '--robust-label-steps', '150']
