"""Proposed method only: full Cora sweep, tau=1 and dropout=.9."""
import argparse
import itertools
import json
...
def main():
    ...
    p.add_argument('--output', default='/content/drive/MyDrive/GRIP_cora_teacher_fisher_full')
    p.add_argument('--raw-data-dir', default='/content/data/')
    p.add_argument('--gammas', default='.001,.01,.1,1')
    p.add_argument('--temperatures', default='.1,.2,.5,1,2,5,10')
    p.add_argument('--mus', default='0,.1,.2,.5,1,2,5,10')
    p.add_argument('--alphas', default='.05,.1,.3,.5,.8,.95')
    p.add_argument('--selection-seeds', default='66,67,68')
    p.add_argument('--confirmation-seeds', default='69,70,71,72,73,74,75,76,77,78')
    ...
    gammas, temps, mus, alphas = [parse(getattr(args, k)) for k in ('gammas', 'temperatures', 'mus', 'alphas')]
    selection, confirmation = parse(args.selection_seeds, int), parse(args.confirmation_seeds, int)
    if (not np.isfinite(gammas + temps + mus + alphas).all() or min(gammas + temps) <= 0
            or min(mus) < 0 or min(alphas) <= 0 or max(alphas) >= 1
            or min(selection + confirmation) < 0 or min(len(selection), len(confirmation)) < 2
            or set(selection) & set(confirmation) or not 1 <= args.eval_every <= args.epoch):
        p.error('Invalid grids, epochs or overlapping seeds')
    ...
    for folder in ('teachers', 'condensed', 'runs', 'models'):
        (out/folder).mkdir(parents=True, exist_ok=True)
    logfile = out / f'full_{datetime.now():%Y%m%d_%H%M%S}.log'
    total = len(gammas) * len(temps) * len(mus) * len(alphas)
    print('GPU:', torch.cuda.get_device_name(0) if args.device.startswith('cuda') else 'CPU')
    print(f'Results: {out}\nLog: {logfile}')
    print(f'Cora .052; tau=1, dropout=.9; {total} settings, {total*len(selection)} selection fits.')
    try:
        from IPython import get_ipython
        from IPython.display import display
        handle = display('Preparing teacher Fisher full sweep...', display_id=True) if get_ipython() else None
    except ImportError:
        handle = None
    ...
    with logfile.open('w', encoding='utf-8') as log:
        ...
        helpers = {name: sha(Path(__file__).with_name(name)) for name in (
            'teacher_fisher_full.py', 'teacher_fisher_metric.py', 'diagonal_metric.py',
            'fisher_grip.py', 'fisher_diagnostic.py')}
        base = dict(source=code_digest(), helpers=helpers, data=data_digest(data),
                    dataset='cora', ratio=.052, kernel='relu', basis=3000,
                    condensation_seed=0, torch=str(torch.__version__), device=args.device,
                    threads=4, metric_temperature=METRIC_TEMPERATURE)
        atomic_json(out/'manifest.json', dict(base=base, arguments=vars(args),
            grids=dict(gamma=gammas, T=temps, mu=mus, alpha=alphas),
            selection_seeds=selection, confirmation_seeds=confirmation))
        ...
        for gamma in gammas:
            ...
            for T, mu, alpha in itertools.product(temps, mus, alphas):
        ...
        winners = {d: min((r for r in summaries if r['domain'] == d),
                          key=lambda r: (-r['val'], r['gamma'], r['T'], r['mu'], r['alpha'])) for d in DOMAINS}
        atomic_json(out/'selection.json', dict(winners=winners,
            entries={d: entries[r['id']] for d, r in winners.items()},
            rule='maximum mean validation; ties: ascending gamma,T,mu,alpha; separate evaluation domains'))
