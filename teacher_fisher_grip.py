"""Sweep teacher diagonal Fisher scaling, with GRIP and student-metric controls."""
import argparse
import json
...
ALPHAS = (0., .05, .1, .3, .5, .8, .95)
SETTINGS = [('grip', 0., 'none'), ('student_0.5', .5, 'student')] + [
    (f'teacher_{a:g}', a, 'teacher') for a in ALPHAS[1:]]
VARIANTS = tuple(item[0] for item in SETTINGS)
...
def main():
    ...
    p.add_argument('--seeds', default='56,57,58,59,60,61,62,63,64,65')
    ...
    try:
        ...
        handle = display('Preparing teacher Fisher sweep...', display_id=True) if get_ipython() else None
    except ImportError:
        handle = None
    ...
    with logfile.open('w', encoding='utf-8') as log:
        ...
        if metric_path.exists():
            ...
            with redirect_stdout(log), redirect_stderr(log):
                teacher_metric = teacher_fisher(z, data.y, data.train_mask, teacher, source_config)
                metric = dict(teacher=teacher_metric, student=sensitivity(z, weights))
            atomic_torch(metric_path, metric)
        print(f'Teacher reconstruction max probability error: '
              f'{metric["teacher"]["source_probability_max_error"]:.3g}')
        for variant, alpha, metric_kind in SETTINGS:
            conf = dict(**base, variant=variant, alpha=alpha, metric_kind=metric_kind, metric_sha256=sha(metric_path))
            ...
            if path.exists():
                ...
                with redirect_stdout(log), redirect_stderr(log):
                    if variant == 'grip':
                        ...
                        seed_everything(0)
                        artifact = condense(z, teacher, metric[metric_kind]['score'], alpha, len(c0),
    ...
    if handle:
        handle.update('Complete. Teacher Fisher alpha sweep below.')
    else:
        print()
    ...
    for domain in PROTOCOLS:
        ...
        for variant in VARIANTS[1:]:
            comparisons.append(dict(domain=domain, variant=variant, baseline='grip',
                ...
                bonferroni_test=paired_stats(by_variant[variant][:, 1], by_variant['grip'][:, 1], 2 * (len(VARIANTS) - 1))))
    atomic_json(out/'summary.json', dict(results=summaries, comparisons=comparisons,
        ...
        'Pointwise student-seed intervals; all nonbaseline-vs-GRIP contrasts also have Bonferroni intervals. '
