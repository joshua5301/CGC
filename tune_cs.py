...
#   stage 1 (Cell 2)  full factorial  kernel/gamma {erf: 1, 3, 10, 30, 100 | relu1: 0.3, 1, 3, 10}  x  T {1, 0.5, 0.25}
#                     x fn {0, 1} x space {raw, nngp} x mu {0.3, 1}                              = 216 condensations
#                     each: dropout {0,.1,.3,.5,.7,.9} x wd {5e-4, 5e-3} on val, repeat 3, eval every 10 epochs (~50 s)
#   stage 2 (Cell 3)  around the top-5 val configs: mu {0.1, 3}, depth 3, lr 5e-3 / 2e-2           ~ 30 more
...
KG      = [('erf', g) for g in (1.0, 3.0, 10.0, 30.0, 100.0)] + [('relu1', g) for g in (0.3, 1.0, 3.0, 10.0)]
TEMPS   = [1.0, 0.5, 0.25]
FNS     = [0, 1]
SPACES  = ['last', 'nngp']
MUS     = [0.3, 1.0]
...
KEYS = ['kernel', 'gamma', 'temp', 'fn', 'space', 'mu', 'depth', 'lr', 'seed']
...
def run(cfg, down, repeat, stage):
    kernel, gamma, temp, fn, space, mu, depth, lr, seed = cfg
    cmd = (f"python main.py {BASE} --ratio {RATIO} --label_kernel {kernel} --gamma {gamma} --teacher_temp {temp} "
           f"--feat_norm {fn} --cluster_feat {space} --bregman {mu} --conv_depth {depth} --lr {lr} --seed {seed} "
    ...
    with open(LOG, 'a') as f:
        for do_, wd_, lr_, te, sd, va in rows:
            f.write(json.dumps(dict(ds='citeseer', ratio=RATIO, kernel=kernel, gamma=gamma, temp=temp, fn=fn, space=space,
                                    mu=mu, depth=depth, lr=lr, seed=seed,
    ...
    print(f"citeseer {RATIO:g} {kernel:5s} g={gamma:<4g} T={temp:<4g} fn={fn} {space:4s} mu={mu:<3g} d={depth} lr={lr:g} s={seed} [{stage}]  "
...
cfg_of = lambda b: (b.kernel, float(b.gamma), float(b.temp), int(b.fn), b.space, float(b.mu), int(b.depth), float(b.lr), int(b.seed))
GRID = [(k, g, T, fn, sp, mu, 2, 0.01, 0) for (k, g), T, fn, sp, mu in itertools.product(KG, TEMPS, FNS, SPACES, MUS)]
...
# ============ Cell 3: stage 2 - neighbourhood of the top-5: mu {0.1, 3}, depth 3, lr {5e-3, 2e-2} (logged as stage1) =============
for _, b in top(5).iterrows():
    k_, g_, T_, fn_, sp_, mu_, d_, lr_, s_ = cfg_of(b)
    for cfg in ([(k_, g_, T_, fn_, sp_, m, 2, 0.01, 0) for m in MU2] + [(k_, g_, T_, fn_, sp_, mu_, 3, 0.01, 0)]
                + [(k_, g_, T_, fn_, sp_, mu_, 2, lr, 0) for lr in (5e-3, 2e-2)]):
        if not done(cfg, 'stage1'):
            run(cfg, DOWN1, REP1, 'stage1')
b = top(1).iloc[0]
...
for ax in ['kernel', 'gamma', 'temp', 'fn', 'space', 'mu', 'depth', 'lr', 'drop', 'wd']:
...
print('\n##### teacher test accuracy per kernel x gamma x fn')
...
for stage, title in [('final', 'MAIN ROWS - top-3 val configs at repeat 10 (first row per ratio = val-best)'),
    ...
    s = s.assign(cell=s.apply(lambda x: f"{x['test']:.1f}+-{x['std']:.1f} (val {x['val']:.1f})", axis=1),
                 cfg=s.apply(lambda x: f"{x['kernel']} g={x['gamma']:g} T={x['temp']:g} fn={x['fn']} {x['space']} mu={x['mu']:g} d={x['depth']} lr={x['lr']:g} s={x['seed']} do={x['drop']:g} wd={x['wd']:g}", axis=1))
    print(f'\n##### {title}')
