...
def run(ds, r, beta, gamma, repeat, tag='', basis=1000, mode='logistic_mean'):
    base = BASE.replace('--label_mode logistic_mean', f'--label_mode {mode}')
    ...
    m = PAT.search(out)
    if not m:
        print('FAIL', ds, r, beta, gamma, basis, mode, '\n', out[-1500:]); return None
    rec = dict(ds=ds, ratio=r, beta=beta, gamma=gamma, basis=basis, mode=mode, repeat=repeat, tag=tag,
    ...
    print(f"{ds:9s} r={r:<7g} {mode:13s} basis={basis:<5} beta={beta:<7g} gamma={gamma:<6g}  "
...
def done(ds, r, beta, gamma, tag, basis, mode='logistic_mean'):
    df = load()
    if not len(df): return False
    if 'mode' not in df: df['mode'] = 'logistic_mean'
    return ((df.ds == ds) & (df.ratio == r) & (df.beta == beta) & (df.gamma == gamma)
            & (df.basis == basis) & (df.tag == tag) & (df['mode'] == mode)).any()
...
for ds in MINE:
    for _, row in best_by_val(ds).iterrows():
        md = row.get('mode', 'logistic_mean')
        if not done(ds, row.ratio, row.beta, row.gamma, 'final', row.basis, md):
            run(ds, row.ratio, row.beta, row.gamma, 10, 'final', int(row.basis), md)

# ============ Cell 4: 메인 표 (어느 세션에서나) =============
df = load(); fin = df[df.tag == 'final'].copy()
fin['cell'] = fin.apply(lambda x: f"{x.test:.1f}±{x.std:.1f}", axis=1)
fin['cfg'] = fin.apply(lambda x: f"{x.get('mode','logistic_mean')} b={x.basis:g} β={x.beta:g} γ={x.gamma:g}", axis=1)
print(fin.pivot(index='ds', columns='ratio', values='cell').to_string())
print(fin.pivot(index='ds', columns='ratio', values='cfg').to_string())
print(fin[['ds', 'ratio', 'mode', 'basis', 'beta', 'gamma', 'val', 'test', 'std']].to_string(index=False))
