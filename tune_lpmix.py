...
# GIFT-style label mixing with propagated hard labels: P_t <- (1-g) f_t + g LP_t, LP = label propagation of the training
# one-hots only (alpha 0.9, 50 iters), applied to the kernel teacher (H = A^2 X) before the temperature, on both the KL
# refinement and the labels (--refine_teacher kernel). norm: LP rows on the simplex; raw: row mass as confidence.
#   stage 1  teacher only: LP alone, and the mixed teacher's val/test for g x mode (cheap, --teacher_only)
#   stage 2  students at cora 1.3 / 2.6 / 5.2 % for g {0, 0.1, 0.3, 0.5} x mode {norm, raw} x T {1, 0.5}, repeat 5
...
GS, MODES, TEMPS = [0.0, 0.1, 0.3, 0.5], ['norm', 'raw'], [1.0, 0.5]
ALPHA = 0.9
...
def teacher(gamma, g, mode):
    ...
    lp = PAT_L.search(out)
    if 'refine' not in pr:
        print('FAIL teacher', gamma, g, mode, '\n', out[-1500:]); return
    rec = dict(stage='teacher', gamma=gamma, g=g, mode=mode, t_val=pr['refine'][0], t_test=pr['refine'][1], h_t=pr['refine'][2],
               lp_val=float(lp[1]) if lp else None, lp_test=float(lp[2]) if lp else None, lp_nomass=float(lp[3]) if lp else None)
    with open(LOG, 'a') as f:
        f.write(json.dumps(rec) + '\n')
    print(f"  [teacher] gamma={gamma:<5g} g={g:<4g} {mode:4s}  val {rec['t_val']:.2f} test {rec['t_test']:.2f} H {rec['h_t']:.3f}"
          + (f"   (LP alone val {lp[1]} test {lp[2]}, no-mass rows {lp[3]}%)" if lp else ''))

def student(ratio, gamma, mu, g, mode, temp):
    cmd = (f"python main.py {BASE} --ratio {ratio} --gamma {gamma} --bregman {mu} --teacher_temp {temp} {flags(g, mode)} "
           f"--repeat {REPEAT} --down_grid '{DOWN}'")
    t = time.time()
    out = subprocess.run(cmd, shell=True, capture_output=True, text=True, cwd='/content/CGC'); out = out.stdout + out.stderr
    rows = PAT_D.findall(out); gd = PAT_G.search(out)
    if not rows or not gd:
        print('FAIL', ratio, g, mode, temp, '\n', out[-2000:]); return
    pr = dict((m[1], (float(m[2]), float(m[3]), float(m[4]))) for m in PAT_P.finditer(out))
    ...
    with open(LOG, 'a') as f:
        for do_, wd_, lr_, te, sdv, va in rows:
            f.write(json.dumps(dict(stage='student', ratio=ratio, gamma=gamma, mu=mu, g=g, mode=mode, temp=temp,
    ...
    print(f"cora {ratio:<6g} g={g:<4g} {mode:4s} T={temp:<4g} teacher val/test {ta[0]}/{ta[1]}  agree {gd[2]}%  H(q) {gd[4]}  "
          f"val {best[5]} (do={best[0]}) test {best[3]}±{best[4]}  ({round(time.time() - t)}s)")

print('lpmix1: GIFT-style mixing with propagated hard labels, cora')
...
# ============ Cell 3: students =============
for (ratio, gamma, mu), g, mode, temp in itertools.product(DENS, GS, MODES, TEMPS):
    if g == 0 and mode == 'raw':
        continue
    if not done(stage='student', ratio=ratio, g=g, mode=mode, temp=temp):
        student(ratio, gamma, mu, g, mode, temp)

# ============ Cell 4: tables =============
df = load(); st = df[df.stage == 'student'].copy()
st['variant'] = st.apply(lambda x: 'none' if x.g == 0 else f"g{x.g:g} {x['mode']}", axis=1)
order = ['none'] + [f"g{g:g} {m}" for g in GS if g > 0 for m in MODES]
...
for temp in TEMPS:
    print(f'\n##### student test per ratio x variant   [T {temp:g}, val-selected dropout, wd 5e-4, repeat 5]')
    print(bt[bt.temp == temp].pivot_table(index='ratio', columns='variant', values='cell', aggfunc='first').reindex(columns=order).to_string())
...
print('\n##### student test per ratio x variant, T selected on val')
...
for m, title in [('t_test', 'mixed teacher test accuracy'), ('agree', 'cell argmax agreement [%]'), ('h_q', 'H(q)'), ('kl_g', 'group KL(true||q)')]:
    print(f'\n##### {title}   [T 1]')
    print(bt[bt.temp == 1.0].pivot_table(index='ratio', columns='variant', values=m, aggfunc='first').reindex(columns=order).round(3).to_string())
